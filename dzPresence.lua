...
--------------------------------------------------------------------------------
local devices_info = {
    {"192.168.8.51", 11925, 30}, -- Array with devices
    {"192.168.8.52", 11926, 3}, -- IP-address, device_idx, cooldown period in minutes
    {"Anyhome", 11938, 0}
} -- Use "Anyhome" for a 'anyone home'-switch
...
local pingcommand = "ping -n 3 -w 3000" -- ping command, no need to change. Script processes received responses, only summary results are required, ping for 5 seconds.
...
return {
    ...
    execute = function(domoticz, item)
        ...
        local function dlog(text)
            return domoticz.log(text, domoticz.LOG_DEBUG)
        end -- DEBUG log
        local function ilog(text)
            return domoticz.log(text, domoticz.LOG_INFO)
        end -- INFO log
        local function flog(text)
            return domoticz.log(text, domoticz.LOG_FORCE)
        end -- Forced (always log regardless of logging setting)
        local function elog(text)
            return domoticz.log(text, domoticz.LOG_ERROR)
        end -- Error logging
        ...
        if item.isTimer then
            ...
            for i = 1, #devices_info do
                ...
                if string.match(ip, "%d+%.%d+%.%d+%.%d+") ~= nil then -- ip looks like ip-address (not anyhome)
                    local shellcommand = 'cmd /C "' .. pingcommand .. " " .. ip .. '"' -- Using cmd /C to allow shell features like | to work (if needed in the future)
                    ilog(shellcommand)
                    domoticz.executeShellCommand(
                        {
                            command = shellcommand,
                            callback = scriptVar .. i, -- dzPresence1, dzPresence2, etc.
                            timeout = 20 -- timeout in seconds before closing shell (in case of no shell output)
                        }
                    )
                end
            end
        elseif item.isShellCommandResponse then
            ...
            if item.statusCode == 0 then -- process result (in item.json, -item.xml, -item.lines or item.data)
                dlog(item.data)
                local pingSuccess = string.match(item.data, "bytes=%d+")
                dlog(pingSuccess)
                if pingSuccess then
                    successcount = 1
                else
            ...
            if successcount > 0 then
                if domoticz.devices(device_idx).state ~= "On" then
            ...
            elseif domoticz.devices(device_idx).lastUpdate.minutesAgo >= device_cooldown then
                if domoticz.devices(device_idx).state ~= "Off" then
                        ...
                        domoticz.devices(device_idx).name ..
                            " (currently " ..
                                domoticz.devices(device_idx).state ..
                                    "), last update " ..
                                        domoticz.devices(device_idx).lastUpdate.minutesAgo .. " minutes ago"
            )
            ...
            if any_idx ~= -1 then -- if idx is saved then set anyhome
                if anyhome and domoticz.devices(any_idx).inActive then
                    if domoticz.devices(any_idx).state ~= "On" then -- switch On if Off and any device is present
                        domoticz.devices(any_idx).switchOn()
                    end
                end
                if not anyhome and domoticz.devices(any_idx).active then
                    if domoticz.devices(any_idx).state ~= "Off" then -- switch Off if On and no device is present
